...
$\mathrm{i}\hbar \frac{\partial\Phi_k}{\partial t}= \Bra{\Phi_{k'}} \hat{T}_e\Ket{\Phi_{k'}} \Phi_k + \Bra{\chi\Phi_{k'}} \hat{V}_{n-e} \Ket{\chi\Phi_{k'}} \Phi_k, \quad k,k'=1...N \wedge k'\neq k $\\    
   \end{tabular} }; 
    ...
    \node[op,below=of scfsinglepp] (ehrenfestmd){\small \begin{tabular}{l}
    $M_I\ddot{\mathbf{R}}_I=-\nabla_IV_e^{E}(\mathbf{R}_I(t))$\\
    $\mathrm{i}\hbar\frac{\partial \Phi}{\partial t}=\hat{H}_{el}(\{\mathbf{r}_i\},\{\mathbf{R}_I(t)\})\Phi(\{\mathbf{r}_i\},\{\mathbf{R}_I(t)\};t)$
    \end{tabular}
    
    };
    
    \node[op,below=of ehrenfestmd,yshift=-2cm] (carparinello){
    \small \begin{tabular}{l}
    $M_I\ddot{\mathbf{R}_I}=-\nabla_I\Bra{\Psi_0}\hat{H}_{el}^{KS}\Ket{\Psi_0}$\\
    $\mu\ddot{\phi}_i(t)=-\hat{H}_{el}^{KS}\phi_i+\sum_j\Lambda_{ij}\phi_j$
    \end{tabular} };
     ...
     $\left[-\sum_I\frac{\hbar^2}{2M_I}\nabla_I^2+E_{el}(\{\mathbf{R}_I\})\right]\chi_m+\sum_p\hat{C}_{mp}\chi_p=E_k\chi_m$
     \end{tabular} };
	...
	$\frac{\partial A_m}{\partial t}+\sum_I\frac{1}{M_I}(\nabla_IA_m)(\nabla_IS_m)+\sum_I\frac{1}{2M_I}A_m(\nabla_I^2S_m)=0$
     ...
     \node[dt,left=of bomd,xshift=0.4cm] (boextension) {
     \begin{tabular}{l}
     -introducing artificiality\\
     -use pseudopotentials\\
     $\vdots$
     \end{tabular} };
     
     \node[dt,below=of boextension] (framework) {
     \begin{tabular}{l}
     Hartree-Fock\\
     Kohn-Sham\\
     Rothaan-Hall\\
    ...
    %right side
    \draw[arrw] (tdse) -- node[xshift=1cm,yshift=0.3cm]{$\Psi_k=\sum_{m=0}^{\infty}\chi_{k,m}\Psi_{el,m}$} ++(6,0) -| (eigenfunc);
    ...
    \draw[arrw] (bomd) -- ++(0,-2.5)|- (framework);
    \draw[arrw,red] (boextension) -- (bomd);
    %left side
    ...
    \draw[arrw] (scfsinglepp) -- node[xshift=0.3cm,yshift=0]{classical limit $\hbar \to 0$} (ehrenfestmd);
    \draw[arrw,red] (boextension) -- ++(-2.8,0) -| ++(0,1.5)|- (ehrenfestmd);
    \draw[arrw] (ehrenfestmd) -- ++(0,-1)|- ++(4,0)-| ++(0,-3.5)|- (framework);
    \draw[arrw] (ehrenfestmd) --node[xshift=0.35cm,yshift=0cm]{Car-Parinello} node[yshift=-0.5cm]{$\phi_{i,t+1}=\phi_{i,t}+\delta\phi$} (carparinello);
